...
fs.readdirSync('./commands')
  ...
  .forEach(dir => {
    ...
    if (pull.config.name) return console.log(`[prefix] Couldn't load the file ${file}, missing module name value.`)
  ...
  if (command) {
    ...
    if (command.owner, command.owner == true) {
      if (config.owners) {
        ...
        if (!config.owners.some(ID => message.member.id.includes(ID))) return message.reply({
          embeds: [
            new EmbedBuilder()
